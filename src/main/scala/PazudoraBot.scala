...
object PazudoraBot extends TelegramBot(
  ...
  on("stat", "몬스터의 스테이터스를 출력합니다.") { (sender, args) =>
    replyTo(sender, parseMode = Some("Markdown")) {
      output(args, mon => s"${mon.getNameString}\n${mon.getStatString}")
    }
  }
  on("rank", "몬스터의 랭크를 출력합니다.") { (sender, args) =>
    replyTo(sender, parseMode = Some("Markdown")) {
      output(args, mon => s"${mon.getNameString}\n${mon.getRanking}")
    }
  }
  on("info", "몬스터의 상세 정보를 츨력합니다.") { (sender, args) =>
    replyTo(sender, parseMode = Some("Markdown")) {
      output(args, mon => s"${mon.getNameString}\n\n${mon.getInfoString}")
    }
  }
  on("pic", "몬스터의 일러스트를 출력합니다.") { (sender, args) =>
    replyTo(sender, parseMode = Some("Markdown")) {
      output(args, mon => s"[${mon.getNameString}](${mon.picURL})")
    }
  }
  on("show", "몬스터의 일러스트와 스테이터스를 출력합니다.") { (sender, args) =>
    replyTo(sender, parseMode = Some("Markdown")) {
      output(args, mon => s"[${mon.getNameString}](${mon.picURL})\n${mon.getStatString}")
    }
  }
  on("roll", "랜덤하게 몬스터를 하나 뽑습니다.") { (sender, args) =>
    replyTo(sender, parseMode = Some("Markdown")) {
      val id = eggs.pick()
  ...
  on("calc", "예상 데미지를 계산합니다.") { (sender, args) =>
    replyTo(sender, parseMode = Some("Markdown")) {
  ...
  on("version", "버전 정보를 출력합니다.") { (sender, args) =>
    replyTo(sender, parseMode = Some("Markdown")) {
  ...
  on("admin", "레어에그 및 내부구조를 관리합니다.") { (sender, args) =>
    replyTo(sender, parseMode = Some("Markdown")) {
      handleAdminCmds(args)
    }
  }
  on("help", "도움말을 출력합니다.") { (sender, args) =>
    replyTo(sender, parseMode = Some("Markdown")) {
