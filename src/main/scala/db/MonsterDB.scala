...
import data.Monster
import data.Monster.{ActiveSkill, AwokenSkill}

/**
  * Created by heejong.lee on 3/9/16.
  */
object MonsterDB extends TIGParser with PDXLeaderSkillParser {
  var cache : Map[MonsterID,Monster] = Map.empty
  def getMonster(id: MonsterID) : Monster = {
    val cachedMon = cache.get(id)
  ...
  private def parseMonster(id: MonsterID) : Monster = {
    ...
    val lskill =
      ...
      else Some(getLeaderSkill(id,rawLSkill(1),rawLSkill(2)))
